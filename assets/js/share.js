    ...
    if (Array.isArray(item.platforms) && item.platforms.length) {
      platformsHTML = '<div class="sc-platforms">' +
        item.platforms.map(function (p) {
          ...
          return icon ? '<span class="sc-platform" title="' + utils.escapeHTML(p) + '">' + icon + "</span>" : "";
        }).join("") + "</div>";
    }
